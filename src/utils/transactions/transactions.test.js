...
describe('retrieve', () => {
  ...
  it('calls retrieveData with correct params', done => {
    retrieve({ AMId: 1, resourceId: 'testID', query: { version: 2 } }, (error, result) => {
      expect(network.retrieveData).toHaveBeenCalledWith({ AMaaSClass: 'transactions', AMId: 1, resourceId: 'testID', query: { version: 2 } })
      done()
    })
  })
})
